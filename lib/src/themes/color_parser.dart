  ...
  static Color? toColor(String? color) {
    ...
    if ((color.startsWith('hsla(') || color.startsWith('hsl(')) &&
      ...
      if (components.length == 4 || components.length == 3) {
        ...
        final saturation = double.tryParse(
          components[1].replaceAll(RegExp(r'%'), ''),
        );
        final lightness = double.tryParse(
          components[2].replaceAll(RegExp(r'%'), ''),
        );
        final alpha = components.length == 3
            ? 1.0
            : alphaValueToDouble(components[3]);
