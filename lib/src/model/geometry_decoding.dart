...
Iterable<Path> decodeLineStringsWithCursor(List<int> geometry) sync* {
  // Cursor point.
  ...
  while (it.moveNext()) {
    ...
    lineString.moveTo(cx.toDouble(), cy.toDouble());
    ...
    for (var i = 0; i < lineSegments; i++) {
      ...
      it.moveNext();
      cy += _decodeZigZag(it.current);
      lineString.lineTo(cx.toDouble(), cy.toDouble());
    }
...
Iterable<Path> decodePolygonsWithCursor(List<int> geometry) sync* {
  Path? polygon;
  ...
  while (it.moveNext()) {
    ...
    x0 = cx;
    y0 = cy;
    ring.moveTo(cx.toDouble(), cy.toDouble());
    ...
    for (var i = 0; i < ringSegments; i++) {
      ...
      cx = x;
      cy = y;
      ring.lineTo(cx.toDouble(), cy.toDouble());
    }
    a += (cx * y0) - (x0 * cy);
    ...
    if (a.isNegative) {
      ...
      assert(polygon != null);
      polygon!.addPath(ring, Offset.zero);
    } else {
      ...
      polygon = ring..fillType = PathFillType.evenOdd;
    }
  }
