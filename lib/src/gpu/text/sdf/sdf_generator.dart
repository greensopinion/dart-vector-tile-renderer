...
Uint8List generateSDF(Uint8List bitmap, int width, int height, int radius, double cutoff) {
  ...
  final separatorPositions = Float64List(maxDimension + 1);  // z array: separator positions
  final parabolicVertices = Int32List(maxDimension);  // v array: parabola vertices
  ...
  const double infinity = 999999.0;  // Large value representing infinite distance
  ...
  for (int columnIndex = 0; columnIndex < width; columnIndex++) {
    _computeDistanceTransform1D(exteriorDistanceGrid, workingDistances, parabolicVertices, 
                               separatorPositions, columnIndex, width, height, width);
    _computeDistanceTransform1D(interiorDistanceGrid, workingDistances, parabolicVertices, 
                               separatorPositions, columnIndex, width, height, width);
  }
  ...
  for (int rowIndex = 0; rowIndex < height; rowIndex++) {
    ...
    _computeDistanceTransform1D(exteriorDistanceGrid, workingDistances, parabolicVertices, 
                               separatorPositions, rowStartOffset, width, 1, 1);
    _computeDistanceTransform1D(interiorDistanceGrid, workingDistances, parabolicVertices, 
                               separatorPositions, rowStartOffset, width, 1, 1);
  }
  ...
  for (int i = 0; i < totalPixels; i++) {
    ...
    final signedDistance = math.sqrt(exteriorDistanceGrid[i]) - math.sqrt(interiorDistanceGrid[i]);
    
    // Normalize to 0-255 range with cutoff bias
    // cutoff shifts the zero-crossing point, radius controls the gradient scale
    final normalizedValue = math.max(0, math.min(255, (signedDistance / radius + cutoff) * 255));
...
void _computeDistanceTransform1D(Float64List distanceGrid, Float64List workingDistances, 
                                Int32List parabolicVertices, Float64List separatorPositions, 
                                int startOffset, int scanLength, int unusedStart, int stepSize) {
    ...
    if (workingDistances[currentPixel] != infinity) {
      if (envelopeSize == 0) {
        ...
        while (envelopeSize > 0) {
          ...
          final intersectionPoint = ((workingDistances[currentPixel] + currentPixel * currentPixel) - 
                                   (workingDistances[previousVertex] + previousVertex * previousVertex)) / 
          ...
          if (envelopeSize > 0) {
            ...
            separatorPositions[envelopeSize] = 
              ((workingDistances[currentPixel] + currentPixel * currentPixel) - 
               (workingDistances[prevVertex] + prevVertex * prevVertex)) / 
  ...
  for (int pixelPosition = 0; pixelPosition < scanLength; pixelPosition++) {
    ...
    if (currentParabolaIndex < envelopeSize) {
      ...
      distanceGrid[startOffset + pixelPosition * stepSize] = 
        workingDistances[optimalVertex] + horizontalDistance * horizontalDistance;
    }
  }
}