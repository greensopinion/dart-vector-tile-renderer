...
class IconRenderer extends SymbolIcon {
  ...
  List<_Segment> _fitContent(
    Sprite sprite,
    double scale, {
    required Size contentSize,
  }) {
    final completeSpriteSource = Rect.fromLTWH(
      sprite.x.toDouble(),
      sprite.y.toDouble(),
    ...
    double spriteContentWidth = (spriteContent[2] - spriteContent[0])
        .toDouble();
    double spriteContentHeight = (spriteContent[3] - spriteContent[1])
        .toDouble();
    double desiredContentWidth = contentWidth + (2 * margin);
    double desiredContentHeight = contentHeight + (2 * margin);
    double desiredScale =
        max(
          desiredContentWidth / spriteContentWidth,
